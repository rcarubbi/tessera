...
The CLI SHALL provide `tessera rules <rules.yaml> validate [dir]` that parses the rules YAML and evaluates it against the report's graph, printing violations (rule, edge, paths, lines) and an exit code of 0 when none, non-zero when violations exist.
